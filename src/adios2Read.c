...
void adios2Read(double* iodata, char* FILENAME, struct params *ioParams)
{
	...
	adios2_variable *iodata_read =  adios2_inquire_variable(ioParams->io, "iodata"); 
	...
	errio = adios2_finalize(ioParams->adios);
	error_check(errio); 
#ifndef NDEBUG
	fprintf(ioParams->debug, "adios2Write-> adios2 object finalised\n");
#endif
} 

...
void ioServer(MPI_Comm ioComm, MPI_Comm newComm, struct params *ioParams)
{
	...
	do 
	{
		...
		for(int i = 0; i < NUM_WIN; i++)
		{
			if(wintestflags[i] > WIN_TEST) // anything over 0 means go for printing 
			{
				/* 
			...
			else if(wintestflags[i] == WIN_TEST && flag[i] == 0)
			{
	...
	for(int i = 0; i < NUM_WIN; i++)
	{
		...
		if(flag[i] == 0)
		{
			...
			fileWrite(ioParams, array[i], loopCounter, i); 
#ifndef NDEBUG 
			fprintf(ioParams->debug, "ioServer window:%i before win free reached\n",i); 
#endif 
		} 
